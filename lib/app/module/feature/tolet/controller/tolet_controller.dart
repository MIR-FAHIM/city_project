...
import 'package:cityproject/app/model/booking/models/AreaModel.dart';
import 'package:cityproject/app/model/booking/models/booking_history_model.dart';
import 'package:cityproject/app/model/booking/models/district_model.dart';
import 'package:cityproject/app/model/booking/models/get_notification_model.dart';
import 'package:cityproject/app/model/booking/models/get_review_model.dart';
import 'package:cityproject/app/model/booking/models/listing/disable_date_model.dart';
import 'package:cityproject/app/model/booking/models/listing/filtered_listing_model.dart';
import 'package:cityproject/app/model/booking/models/listing/get_fav_listing_model.dart';
...
class ToletController extends GetxController {
  ...
  final disableDateList = <DisableDate>[].obs;
  final disableDate = <DateTime>[].obs;
  ...
  final numBeds = 0.obs;
  final numBath = 0.obs;
  final caruPage = 0.obs;
  final caruPageFav = 0.obs;
  final dataIndex = 0.obs;
  ...
  final favListingData = <Favourite>[].obs;
  final detailsData = <FilteredListing>[].obs;
  final getReview = <GetReview>[].obs;
  ...
  final rangeStart = DateTime(2024, 1, 27).obs;
  final rangeEnd = DateTime(2024, 1, 27).add(Duration(days: 1)).obs;
  var startDate = ''.obs;
  var endDate = ''.obs;
  ...
  final payStationToken = "".obs;

  // short stey check box value
  final shortStayCheckSlot1 = false.obs;
  final shortStayCheckSlot2 = false.obs;
  final shortStayCheckSlot3 = false.obs;

  final filterListData = <Map<String, dynamic>>[].obs;
  final isFavorite = false.obs;
  final profileData = ProfileModel().obs;
  final notificationList = <NotificationList>[].obs;
  final slotCounter = 0.obs;
  ...
  playVideoController(link) {
    videoController = VideoPlayerController.networkUrl(
      Uri.parse(
          "https://drive.google.com/file/d/1dRf18mbjh_MZQ4G8RpLzJtX8uvm3mfr7/view?usp=sharing"),
    )..initialize().then((_) {
  ...
  ///fav__________________________________******
  ...
  List<FilteredListing> get filteredListingList {
    return listingData.value.where((place) {
      final name =
          place.listingTitle.toLowerCase() + place.listingAddress.toLowerCase();
  ...
  void onMarkerTapped(MarkerId markerId) {
    ...
    Get.toNamed(Routes.TOLETDETAILS);
  }
  ...
  bool isEnabledDay(DateTime day) {
    if(disableDate.contains(DateTime(day.year, day.month, day.day)) == true){
      return false;
    }else{
      return true;
    }
  ...
  bool isDateDisabled(DateTime day) {
    return disableDate.contains(day);
  }
  getDisableDates(listingId) {
    disableDate.clear();
    ListingRep().getDisableDates(listingId).then((e) async {

      if (e['status'] ==  200) {
        DisableDateModel  model  = DisableDateModel.fromJson(e) ;
        disableDateList.value = model.dates!;

        disableDateList.forEach((element) {
          //DateTime(2023, 12, 8);
          disableDate.add(DateTime(
            int.parse(element.dates!.substring(0, 4)),
            int.parse(element.dates!.substring(5, 7)),
            int.parse(element.dates!.substring(8, 10)),
          ));
        });
        print(
            "my all disable date list for $listingId is hlw bro +++++++++++++++${disableDate.value}");
  ...
  void rangeSelector(DateTime? start, DateTime? end, DateTime? focus) {

    bool hasSameDayAndMonth = disableDate.any((dateTime) {
      return getDatesBetween(start!, end??start).any((disabledDate) {
        return dateTime.day == disabledDate.day && dateTime.month == disabledDate.month;
      });
    });

    if(hasSameDayAndMonth){
      print("sorry");
    }else{
  ...
  getDateDifferenceNum() {
    final difference = rangeEnd.value.difference(rangeStart.value).inDays;
    totalStay.value = difference.toString();

    return difference;
  }

  RxInt staySelectedOption = 2.obs;

  void setSelectedOption(int value) {
    staySelectedOption.value = value;
  }
